...
def correr_experimento(criterio, p_de_vacio, iters, seed=100):
    random.seed(seed)

    tiempos = []

    for i in range(iters):
        res = simular(FILAS, N_PASAJEROS, ASIENTOS_VALIDOS, P_CARRY_ON, T_SENTADO, T_CARRYON, VISUALIZAR, criterio, P_DE_VACIO=p_de_vacio)
...
def imprimir_resumen(criterio, p, resumen, n):
    print(f"== {criterio.name} P_DE_VACIO={p} N={n} ==")
    print(f"  Promedio: {resumen['promedio']/60:.2f} min")
    print(f"  Desvío estándar: {resumen['desvio_std']/60:.2f} min")


def guardar_csv(resultados, path="promedios_por_p_de_vacio.csv"):
    nombres = [criterio.name for criterio in resultados]

    with open(path, "w", newline="") as f:
        writer = csv.writer(f)

        writer.writerow(["p_de_vacio"] + nombres)

        for p in VALORES_P:
            fila = [
                p
            ] + [
                resultados[criterio][p]["promedio"]
