...
def posiciones_por_zona(FILAS, N_PASAJEROS, ASIENTOS_VALIDOS, K, reverse=True):
    ...
    for i in range(K):
        temp = posiciones_random(filas_por_grupo + filas_por_grupo*i, ASIENTOS_VALIDOS, pasajeros_por_grupo, desde=filas_por_grupo*i)
        for pos in range(len(temp)):
            posiciones.append(temp[pos])

    temp = posiciones_random(FILAS, ASIENTOS_VALIDOS, resto, desde=filas_por_grupo*K)
    if temp:
        for pos in range(len(temp)):
            posiciones.append(temp[pos])
...
def posiciones_steffen2(FILAS, N_PASAJEROS, ASIENTOS_VALIDOS):
    ...
    # Asientos ventana
    ventanas = [ASIENTOS_VALIDOS[0], ASIENTOS_VALIDOS[-1]]

    # Asientos pasillo
    pasillos = [a for a in ASIENTOS_VALIDOS if a not in ventanas]

    grupos = [
        (ventanas, range(FILAS, 0, -2)),       # ventanas, filas impares
        (ventanas, range(FILAS - 1, 0, -2)),   # ventanas, filas pares
        (pasillos, range(FILAS, 0, -2)),       # pasillos, filas impares
        (pasillos, range(FILAS - 1, 0, -2))    # pasillos, filas pares
    ]

    for asientos, filas in grupos:
        for f in filas:
            for a in asientos:
                posiciones.append((f, a))
